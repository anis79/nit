#include "nit.common.h"
#define COLOR_nit__astvalidation__ANode__accept_ast_validation 24
#define COLOR_nit__astvalidation__ASTValidationVisitor___path 1
extern const char FILE_nit__astvalidation[];
#define COLOR_nit__astvalidation__ASTValidationVisitor___seen 2
#define COLOR_nit__parser_nodes__ANode___parent 1
long standard___standard__List___standard__abstract_collection__Collection__length(val* self);
extern const struct type type_standard__Int;
extern const char FILE_standard__kernel[];
val* standard___standard__List___standard__abstract_collection__Collection__first(val* self);
void standard___standard__HashSet___standard__abstract_collection__SimpleCollection__add(val* self, val* p0);
#define COLOR_standard__kernel__Object___61d_61d 4
short int standard___standard__HashSet___standard__abstract_collection__Collection__has(val* self, val* p0);
val* NEW_standard__NativeArray(int length, const struct type* type);
extern const struct type type_standard__NativeArray__standard__String;
val* standard___standard__NativeString___to_s_with_length(unsigned char* self, long p0);
#define COLOR_standard__string__Object__to_s 3
#define COLOR_standard__string__NativeArray__native_to_s 16
void nit___nit__ANode___debug(val* self, val* p0);
#define COLOR_nit__parser_nodes__ANode___location 0
extern const char FILE_nit__parser_nodes[];
void standard___standard__List___standard__abstract_collection__Sequence__unshift(val* self, val* p0);
#define COLOR_nit__parser_nodes__ANode__visit_all 20
val* standard___standard__List___standard__abstract_collection__Sequence__shift(val* self);
#define COLOR_nit__astvalidation___AExpr___ANode__accept_ast_validation 61
#define COLOR_nit__typing__AExpr__mtype 42
#define COLOR_nit__typing__AExpr___is_typed 6
#define COLOR_nit__astvalidation___APlaceholderExpr___ANode__accept_ast_validation 64
